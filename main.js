...
var _RequestInterceptor = _interopRequireDefault(__webpack_require__(60));
...
var _minilog = _interopRequireDefault(__webpack_require__(26));
var _umd = _interopRequireDefault(__webpack_require__(38));
var _pTimeout = _interopRequireDefault(__webpack_require__(39));
var _pWaitFor = _interopRequireWildcard(__webpack_require__(40));
...
/* harmony export */ });
/* harmony import */ var p_timeout__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(41);
...
var _classCallCheck2 = _interopRequireDefault(__webpack_require__(22));
var _createClass2 = _interopRequireDefault(__webpack_require__(23));
var _microee = _interopRequireDefault(__webpack_require__(29));
var _utils = __webpack_require__(42);
function _createForOfIteratorHelper(o, allowArrayLike) { var it = typeof Symbol !== "undefined" && o[Symbol.iterator] || o["@@iterator"]; if (!it) { if (Array.isArray(o) || (it = _unsupportedIterableToArray(o)) || allowArrayLike && o && typeof o.length === "number") { if (it) o = it; var i = 0; var F = function F() {}; return { s: F, n: function n() { if (i >= o.length) return { done: true }; return { done: false, value: o[i++] }; }, e: function e(_e) { throw _e; }, f: F }; } throw new TypeError("Invalid attempt to iterate non-iterable instance.\nIn order to be iterable, non-array objects must have a [Symbol.iterator]() method."); } var normalCompletion = true, didErr = false, err; return { s: function s() { it = it.call(o); }, n: function n() { var step = it.next(); normalCompletion = step.done; return step; }, e: function e(_e2) { didErr = true; err = _e2; }, f: function f() { try { if (!normalCompletion && it.return != null) it.return(); } finally { if (didErr) throw err; } } }; }
function _unsupportedIterableToArray(o, minLen) { if (!o) return; if (typeof o === "string") return _arrayLikeToArray(o, minLen); var n = Object.prototype.toString.call(o).slice(8, -1); if (n === "Object" && o.constructor) n = o.constructor.name; if (n === "Map" || n === "Set") return Array.from(o); if (n === "Arguments" || /^(?:Ui|I)nt(?:8|16|32)(?:Clamped)?Array$/.test(n)) return _arrayLikeToArray(o, minLen); }
...
/* harmony export */ });
/* harmony import */ var microee__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(29);
 ...
 * Intercept any xhr or fetch request corresponding to the given interception list.
 *
 * Runs in the "worker" context (the Gan web page). Overrides window.fetch and
 * XMLHttpRequest so we can read the JSON responses of Gan's internal API (health
 * reimbursements) and capture the Authorization header of the OIDC-protected calls.
 */
class RequestInterceptor {
  /**
   * @param {Array<InterceptionDocument>} interceptions - the list of urls to intercept
   */
  ...
  /**
   * Install the xhr/fetch overrides.
   */
  init() {
    try {
      const self = this
      window.XMLHttpRequest.prototype.setRequestHeader = function (key, value) {
        try {
          ...
          console.log('❌ xhr setRequestHeader interception error', err.message)
        }
      }
      window.XMLHttpRequest.prototype.open = function (method, url) {
        try {
      ...
      window.fetch = async function (...args) {
        ...
        try {
          ...
          console.log('❌ fetch interception error', err.message)
        }
      }
    } catch (err) {
  ...
  /**
   * Serialize the intercepted response according to the "serialization" attribute of the
   * matching interception and emit it as a "response" event.
   *
   * @param {object} resp
   */
  async serializeAndEmitResponse(resp) {
    ...
    const interception = this.interceptions.find(doc => {
      if (resp.method !== doc.method) return false
      return doc.exact ? resp.url === doc.url : resp.url.includes(doc.url)
    })
...
/**
 * @typedef EmittedResponse
 * @property {string} label - name given to the interception
 * @property {'GET'|'POST'|'PUT'|'DELETE'} method - method of the intercepted request
 * @property {string} url - intercepted request url
 * @property {object} response - deserialized response body
 * @property {object} responseHeaders - response headers
 * @property {object} requestHeaders - request headers
 */
...
/**
 * @typedef InterceptionDocument
 * @property {string} label - name given to the interception, found back in the response
 * @property {string} url - the url (or substring) to intercept
 * @property {'GET'|'POST'|'PUT'|'DELETE'} method - the method to intercept
 * @property {boolean} [exact] - true if the url must match exactly (default: substring match)
 * @property {'json'|'text'|'dataUri'} serialization - how to serialize the body
 */
 ...
 */
function parseDocuments(data, opts = {}, logger) {
...
/* harmony import */ var _cozy_minilog__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(26);
...
/* harmony import */ var p_timeout__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(61);
/* harmony import */ var _interceptor__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(62);
/* harmony import */ var _parsing__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(63);
...
const interceptor = new _interceptor__WEBPACK_IMPORTED_MODULE_3__["default"](INTERCEPTIONS)
interceptor.init()

class GanContentScript extends cozy_clisk_dist_contentscript__WEBPACK_IMPORTED_MODULE_0__.ContentScript {
  // -----------------------------------------------------------------------
  // Wiring
  // -----------------------------------------------------------------------
  async init(options) {
  ...
  onInterceptedResponse(payload) {
    ...
    if (DISCOVERY_MODE) {
      this.log(
        'info',
        `🔎 DISCOVERY ${payload.method} ${payload.url} → ${(0,_parsing__WEBPACK_IMPORTED_MODULE_4__.summarizeJson)(
          payload.response
        )}`
      )
    }
  ...
  async waitForInterceptionSafe(label, { timeout = 60000 } = {}) {
    ...
    try {
      return await (0,p_timeout__WEBPACK_IMPORTED_MODULE_2__["default"])(promise, {
        milliseconds: timeout,
